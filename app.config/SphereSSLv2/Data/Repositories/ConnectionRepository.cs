...
namespace SphereSSLv2.Data.Repositories
{
    public class ConnectionRepository
    {
        ...
        // Returns every connection owned by an Admin or SuperAdmin (for broadcast notifications).
        public async Task<List<UserConnection>> GetAdminConnectionsAsync()
        {
            var list = new List<UserConnection>();
            using var conn = new SqliteConnection($"Data Source={ConfigureService.dbPath}");
            await conn.OpenAsync();
            var cmd = conn.CreateCommand();
            cmd.CommandText = @"
                SELECT c.* FROM UserConnections c
                INNER JOIN UserRoles r ON c.UserId = r.UserId
                WHERE r.Role IN ('Admin', 'SuperAdmin') OR r.IsAdmin = 1";
            using var reader = await cmd.ExecuteReaderAsync();
            while (await reader.ReadAsync())
                list.Add(MapRow(reader));
            return list;
