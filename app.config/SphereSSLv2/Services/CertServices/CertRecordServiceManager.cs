...
namespace SphereSSLv2.Services.CertServices
{
    public class CertRecordServiceManager
    {
                ...
                while (attempt < maxAttempts)
                {
                    ...
                    if (allVerified)
                    {
                        ...
                        if (order.SaveForRenewal)
                        {
                            ...
                            _ = NotificationService.NotifyOwnerAndAdminsAsync(order.UserId, "RenewSuccess",
                                $"SphereSSL: certificate renewed successfully for {_domains} (owner: {username}). New expiry: {order.ExpiryDate:yyyy-MM-dd}.");
                        }
                        else
                        {
        ...
        public async Task<bool> FinishManualRenewCertRecordById(Logger logger, string orderId)
        {
            ...
            while (attempt < maxAttempts)
            {
                ...
                if (allVerified)
                {
                    ...
                    if (order.SaveForRenewal)
                    {
                        ...
                        _ = NotificationService.NotifyOwnerAndAdminsAsync(order.UserId, "RenewSuccess",
                            $"SphereSSL: certificate renewed successfully for {_domains2} (owner: {username}). New expiry: {order.ExpiryDate:yyyy-MM-dd}.");
                    }
                    else
                    {
                        foreach (var __challenge in failedChallenges)
            ...
            _ = NotificationService.NotifyOwnerAndAdminsAsync(order.UserId, "RenewFail",
                $"SphereSSL: certificate renewal FAILED for {_failDomains} (owner: {username}) after {maxAttempts} attempts.");
