...
namespace SphereSSLv2.Services
{
    public static class NotificationService
    {
        ...
        // Notify the cert owner AND every Admin/SuperAdmin. Dedups so admins who own the cert only get one ping.
        public static async Task NotifyOwnerAndAdminsAsync(string ownerUserId, string eventType, string message)
        {
            try
            {
                var seen = new HashSet<string>();
                var combined = new List<UserConnection>();

                if (!string.IsNullOrEmpty(ownerUserId))
                    combined.AddRange(await _connectionRepo.GetConnectionsByUserIdAsync(ownerUserId));

                combined.AddRange(await _connectionRepo.GetAdminConnectionsAsync());
